...
namespace PrisonLabor
{
    public class WorkGiver_Miner_Tweak : WorkGiver_Scanner
    {
        ...
        {
            foreach (var des in pawn.Map.designationManager.SpawnedDesignationsOfDef(DesignationDefOf.Mine))
            {
                ...
                if (mayBeAccessible)
                {
                    var j = MineUtility.MineableInCell(des.target.Cell, pawn.Map);
                    if (j != null)
                        yield return j;
                }
            }
        ...
        public override Job JobOnThing(Pawn pawn, Thing t, bool forced = false)
        {
            ...
            for (var i = 0; i < 8; i++)
            {
                ...
                if (intVec.InBounds(pawn.Map) && intVec.Standable(pawn.Map) &&
                    ReachabilityImmediate.CanReachImmediate(intVec, t, pawn.Map, PathEndMode.Touch, pawn))
                {
                    flag = true;
                    break;
                }
                ...
                for (var j = 0; j < 8; j++)
                {
                    ...
                    if (intVec2.InBounds(t.Map))
                        if (ReachabilityImmediate.CanReachImmediate(intVec2, t, pawn.Map, PathEndMode.Touch, pawn))
                            ...
                            {
                                var firstHaulable = intVec2.GetFirstHaulable(t.Map);
                                if (firstHaulable != null &&
                                    firstHaulable.def.passability == Traversability.PassThroughOnly)
                                    return HaulAIUtility.HaulAsideJobFor(pawn, firstHaulable);
                            }
                }
                return null;
            }
            return new Job(DefDatabase<JobDef>.GetNamed("PrisonLabor_Mine_Tweak"), t, 1500, true);
        }
    }
}