  ...
  public class PrisonerButtonWindow : MainTabWindow
  {
    ...
    public override Vector2 RequestedTabSize
    {
      get
      {
        ...
        foreach (var tab in tabsView.Where(kv => ShouldShowTab(kv.Key)).Select(kv => kv.Value.RequestedTabSize))
        {
          size.x = Math.Max(size.x, tab.x);
          size.y = Math.Max(size.y, tab.y);
        }
    ...
    {
      GetTable(CurTab).Notify_PawnsChanged();
