...
namespace PrisonLabor.Core.AI.JobGivers
{
    public class JobGiver_Labor : ThinkNode
    {
        ...
        public override float GetPriority(Pawn pawn)
        {
            ...
            {
                DebugLogger.debug($"Prisoner {pawn.NameShortColored} labor piority: 0");
                return 0f;
            }
            if (timeAssignmentDef == TimeAssignmentDefOf.Work)
            {
                DebugLogger.debug($"Prisoner {pawn.NameShortColored} labor piority: 10");
                return 10f;
            }
            DebugLogger.debug($"Prisoner {pawn.NameShortColored} labor piority: 8");
            return 8f;
        }
