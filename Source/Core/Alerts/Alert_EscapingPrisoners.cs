...
namespace PrisonLabor.Core.Alerts
{
  public class Alert_EscapingPrisoners : Alert_Critical
  {
    ...
    private IEnumerable<Pawn> PotentialEscapingPrisoners
    {
      get
      {
        return PawnsFinder.AllMaps_PrisonersOfColonySpawned.Where(prisoner => EscapingPrisoner(prisoner));
      }
    }

    private bool EscapingPrisoner(Pawn prisoner)
    {
      PrisonerComp prisonerComp = prisoner.TryGetComp<PrisonerComp>();
      return prisonerComp != null && prisonerComp.EscapeTracker.ReadyToEscape;
    }
