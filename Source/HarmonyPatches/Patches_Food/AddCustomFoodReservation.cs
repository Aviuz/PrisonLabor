  ...
  static class AddCustomFoodReservation
  {
    ...
    static IEnumerable<CodeInstruction> Transpiler(ILGenerator gen, IEnumerable<CodeInstruction> instructions)
    {
      ...
      string[] operands =
      {
        ...
        "System.Predicate`1[Verse.Thing] foodValidator",
      };
      int step = 0;
      ...
      foreach (var instr in instructions)
      {
        if (HPatcher.IsFragment(opCodes, operands, instr, ref step, nameof(AddCustomFoodReservation), false))
        {
          ...
          yield return new CodeInstruction(OpCodes.Call,
            typeof(AddCustomFoodReservation).GetMethod(nameof(AddConditionToPredicate)));
        }
    ...
    public static Predicate<Thing> AddConditionToPredicate(Predicate<Thing> predicate, Pawn getter, Pawn eater,
      bool desperate)
    {
      return target =>
      {
